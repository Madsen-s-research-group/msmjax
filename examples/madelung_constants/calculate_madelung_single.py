...
if __name__ == "__main__":
    ...
    for resultsfile, ax_twin, marker in zip(
        ...
        if onp.allclose(h_1_a, [h_1_b, h_1_c]):
            if onp.isclose(h_1_a / d_min_cation_anion, 1.0):
                label = f"$h_1 = "
                label += r"d_{\text{min}} \, $        "
            else:
                label = f"$h_1 = {h_1_a / d_min_cation_anion:.2f} \,"
                label += r"d_{\text{min}}$ "
        else:
            formatted_scaled_spacings = [
                f"{x / d_min_cation_anion:.2f}" for x in (h_1_a, h_1_b, h_1_c)
            ]
            label = f"$h_1 = ({', '.join(formatted_scaled_spacings)}) \, "
            label += r"d_{\text{min}}$ "
        label += "$\Longrightarrow$ " + f"{n_levels} grid level"
        if n_levels > 1:
            label += "s"
        (graph,) = ax.plot(
            cutoffs_relative,
