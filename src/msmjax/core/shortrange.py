...
from msmjax.jax_md.smap import _generalized_diagonal_mask
...
def make_eval_pair_pot(
    ...
    The design of this implementation is inspired by JAX-MD's `smap.pair`
    and internally uses some of the same lower-level functionality.
    ...
    Args:
        ...
        per_particle: If true, an array of energies per particle is returned
            rather than the total system energy.
        extra_uncharged_interaction: An optional additional charge-unaware
            pair potential that is evaluated along with the MSM's short-range
            kernel, reusing the same distance computations for efficiency,
            and added to the total system energy.
...
def make_eval_pair_pot_neighborlist(
    ...
    The design of this implementation is inspired by JAX-MD's
    `smap.pair_neighbor_list` and internally uses some of the same
    ...
    .. warning::
       ...
       this entails. If the cutoff radius of ``kernel_fn`` is too large for
       the unit cell, or the unit cell is too deformed, results will be
       incorrect. Unlike the no-neighbor-list analog
       :func:`make_eval_pair_pot`, this version does not have the built-in
       supercell generation feature via ``supercell_diag``. This should not be
       a major limitation since in cases where supercell is required, the
       benefit of using a neighbor list is likely to be small, if any.
    ...
    Args:
        ...
        extra_uncharged_interaction: An optional additional charge-unaware
            pair potential that is evaluated along with the MSM's short-range
            kernel, reusing the same neighbor list and distance computations
            for efficiency, and added to the total system energy.
