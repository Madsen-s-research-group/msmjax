...
def make_compute_u_oneplus(
    ...
    Args:
        basis_eval_fn:
            ...
            Inputs and outputs:
                ...
                - Input to ``basis_eval_fn`` should be a
                  1-d array, shape `(n_dim,)`, representing the coordinates
                  of a single particle.

                - Output of ``basis_eval_fn`` should be a
            ...
            .. note::
               ...
               ``basis_eval_fn`` should always return flat arrays.

            .. warning::
               If ``basis_eval_fn`` returns indices that are out of bounds
               w.r.t. to the grid size defined by ``grid_shape_lvl_one``,
               this will result in NaNs. This is intentional because errors
               like particles moving outside the grid boundaries might
        ...
        transform_mode: The presence of this argument signals that positions
            should be transformed to the unit cube before passing them to
            ``basis_eval_fn``, in which case ``basis_eval_fn`` must have been
            defined such that it acts on positions given w.r.t. to the unit
            cube. Its value is a string specifying assumptions on the shape
            of the unit cell used to simplify the transformation.
            Together with appropriate values given for ``basis_eval_fn``,
            and ``kernel_stencils`` or ``kernel_stencil_construction_fn``, this
            argument serves to support non-orthorhombic cells as well as ones
            that can dynamically change.
        kernel_stencils: A sequence of precomputed arrays representing kernel
            stencils for convolution with the grid charge. Use this argument
            when the cell is fixed, and the kernel stencils thus don't change.
            Mutually exclusive with ``kernel_stencil_construction_fn``.
        kernel_stencil_construction_fn:
            A function that computes the sequence of kernel stencils from the
            current cell. Use this argument when the cell can change, such
            that the kernel stencils are not known in advance.
            Mutually exclusive with ``kernel_stencils``.
    ...
    def _calc_energy(
        ...
        """
        basis_vals, basis_inds = jax.vmap(basis_eval_fn)(positions)
    ...
    def _calc_energy_custom_jvp(primals, tangents):
        ...
        basis_grads, basis_inds = jax.vmap(
            jax.jacfwd(basis_eval_fn, has_aux=True)
        )(positions)
