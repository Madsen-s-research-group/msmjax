...
def get_max_cutoff_for_mic(cell: ArrayLike) -> Array:
...
def find_covering_grid_extents(
    ...
    Args:
        grid_axes: Array of shape `(n_dim, n_dim)`, representing the directions
            of the grid axes (rows are different grid axes, columns are
            Cartesian coordinates). Only the directions matter, as the vectors
            are internally normalized, and the grid spacings are supplied
            separately via the ``spacings`` argument.
        spacings: 1-d array of grid spacing, one along each grid axis.
        cutoff: Cutoff radius.

    Returns:
        Tuple of the required numbers of grid cells, for each axis direction,
        needed for the cutoff sphere to be contained in them. They are counted
        from the center of the sphere, i.e., e.g., for a sphere of radius 3.5
        and a grid spanned by orthogonal axes and with a grid spacing of 1.0,
        in three dimensions, the result would be (4, 4, 4).
    """
