...
def calc_Phi(p: int, u: npt.ArrayLike) -> np.ndarray:
    ...
    knots = np.arange(-p // 2, p // 2 + 1)
    out = BSpline.basis_element(knots)(u)
    out[(u < knots[0]) | (u > knots[-1])] = 0.0
    return out


def build_col_of_B(p: int) -> np.ndarray:
...
def build_col_of_B2(p: int) -> np.ndarray:
...
def build_Toeplitz_matrix(p: int, n_rows: int) -> np.ndarray:
...
def build_Toeplitz_system(
    p: int, rhs: np.ndarray, padding: int
...
def compute_c_m(
    ...
    Args:
        ...
        C_coeffs: The right-hand side of the unnumbered equation
...
def generating_function(z: sympy.Expr, s: sympy.Expr) -> sympy.Mul:
...
def calc_Mclaurin_coefficient(
    function: sympy.Function, variable: sympy.Symbol, order: int
) -> sympy.Expr:
...
def calc_B_p_over_2(p: int) -> sympy.Poly:
...
def calc_polynomials(p: int) -> tuple[sympy.Poly, sympy.Poly]:
...
def explode_Add(add_object: sympy.Add) -> dict:
    ...
    Returns:
        A dictionary of coefficients of the powers contained in add_object.
        The keys are the exponents.
...
def coeff_dict_to_array(coefficients: dict) -> np.ndarray:
...
def compute_quasi_omega_prime(
    p: int, mu: int
) -> tuple[np.ndarray, np.ndarray]:
    """Compute the coefficients :math:`\\omega_{\\mu,m}'` from the article.
    ...
    Args:
        ...
        mu: Adjustable parameter in the quasi-interpolation scheme that
            controls its accuracy at grid points.

    Returns:
        Tuple containing:
            - The coefficients denoted $\\omega_{\\mu,m}'$ in the article.
            - The coefficients denoted $c_m$ in the article, in the unnumbered
              equation after Eq. 12 that defines the expansion of the
              operator A.
    """
    ...
    # Build the non-zero "core" of the right-hand side of the unnumbered
    # equation in appendix C that defined the c_m.
    ...
    # The quasi-interpolating omega_prime consist of two contributions: one
    # coming from the "exact" part (the b polynomial) and the other from the
    # truncated c_m. Therefore, we also need to translate the coefficients
    # of b into a NumPy array.
    ...
    quasi_omega_prime = b_coefficients + multiplied_c_m

    return quasi_omega_prime, c_m
